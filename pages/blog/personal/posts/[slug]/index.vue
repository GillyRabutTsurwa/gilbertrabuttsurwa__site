...
const query: string = post(url);
const { data: content } = await useSanityQuery<Post>(query);
...
<template>
  <article class="blog-container">
    <figure class="blog-img-container" v-if="!showElement">
      <SanityImage :asset-id="content?.asset?._ref" auto="format" />
    </figure>

    <div class="blog-content">
      <Button isLink path="/blog/personal/posts/" text="All Posts" colourPrimary="#104f55" colourSecondary="#f0f0f0" />
      <h1 class="blog-content__title">{{ content.title }}</h1>
      <h3 class="blog-content__author">
        <span>By: </span>
        <NuxtLink :to="`/authours/${content.author.slug.current}`">
